...
class read_data():
    def __init__(self):
        ...
        self.PRODUCTS_PATH = os.path.join(self.ASSETS_DIR, "PRODUCTS.csv")
        self.PRICES_STOCK_PATH = os.path.join(self.ASSETS_DIR, "PRICES-STOCK.csv")
        self.DB_DIR = os.path.join(self.PROJECT_DIR, 'db.sqlite')
        self.engine = create_engine(r'sqlite:///' + self.DB_DIR)
        print(os.path.isfile(self.DB_DIR))
        if(os.path.isfile(self.DB_DIR) is not True):
    ...
    def conv(self,df, *args):
        for col in args:
            df[col] = df[col].astype('category')


    def capitalize(self,df, col_names):
        for col in col_names:
            df[col] = df[col].str.capitalize()


    def lower_col_record(self,df, col_names):
        for col in col_names:
    ...
    def process_csv_files(self):
        products_df = pd.read_csv(filepath_or_buffer=self.PRODUCTS_PATH, sep="|")
        prices_stock_df = pd.read_csv(filepath_or_buffer=self.PRICES_STOCK_PATH, sep="|")
        ...
        self.capitalize(products_df, ['NAME', 'BRAND', 'DESCRIPTION'])
        self.lower_col_record(products_df, ['CATEGORY', 'PACKAGE'])
        self.lower(products_df, branchproducts_df)
        ...
        sql_df = pd.read_sql("SELECT products.id, sku FROM products WHERE store = 'Richarts'", con=self.engine)
